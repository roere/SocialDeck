...
expectTokenError('Token Scope-Satz abgelehnt','LINKEDIN_SCOPE_DENIED',tokenResult(400,'{"error":"invalid_scope","error_description":"unit-private-description"}'));
expectTokenError('Token-Warnung ohne Rohdaten','LINKEDIN_TOKEN_TIMEOUT',tokenResult(0,false,'Connection timed out unit-private-description'));
$contents=(string)file_get_contents($log);foreach(['unit-secret-never-log','unit-code-never-log','unit-access-never-log','unit-private-description','linkedinErrorDescription','transportErrorMessage'] as $forbidden)if(str_contains($contents,$forbidden))throw new RuntimeException('Secret in Token-Log');foreach(['"phase":"token_exchange"','"httpStatus":400','"oauthError":"invalid_grant"','"oauthErrorCategory":"token_exchange"','"transportError":"tls"','"requestedScopes":[]'] as $required)if(!str_contains($contents,$required))throw new RuntimeException("Diagnosefeld fehlt: $required");echo "PASS Token-Logs strukturiert und secretfrei\n";
