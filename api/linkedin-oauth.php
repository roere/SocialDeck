...
require_once __DIR__.'/linkedin-oauth-result.php';
...
function linkedInOAuthDiagnostic(string $phase,?string $state=null,?array $saved=null,?string $result=null): void {
    error_log('SocialPost LinkedIn OAuth '.json_encode(['phase'=>$phase,'requestedScopes'=>linkedInSafeScopeNames($saved['requestedScopes']??[]),'result'=>$result],JSON_UNESCAPED_SLASHES));
}
function activeLinkedInOAuthStates(int $now): array {
    $states=$_SESSION['linkedin_oauth_states']??[];if(!is_array($states))$states=[];
    foreach($states as $hash=>$metadata){$createdAt=is_array($metadata)?($metadata['created_at']??null):$metadata;if(!is_string($hash)||strlen($hash)!==64||!is_int($createdAt)||$createdAt<$now-600||$createdAt>$now+30)unset($states[$hash]);}
    return array_slice($states,-4,null,true);
}
function linkedInConfig(?int $appId=null): array {
    $row=$appId?providerAppRow('linkedin',$appId):soleProviderApp('linkedin');
    ...
    $scopes=array_values(array_unique(array_filter(array_map('trim',preg_split('/[\s,]+/',(string)$row['configured_scopes'])))));
    ...
    return ['providerAppId'=>(int)$row['id'],'appName'=>$row['name'],'credentialVersion'=>hash('sha256',$row['client_id'].'|'.$row['client_secret_encrypted'].'|'.$row['redirect_uri']),'clientId'=>trim($row['client_id']),'clientSecret'=>$secret,'redirectUri'=>trim($row['redirect_uri']),'scopes'=>$scopes];
}
...
function linkedInTokenExchange(array $config,string $code,?callable $transport=null): array {
    ...
    error_log('SocialPost LinkedIn OAuth '.json_encode(['phase'=>'token_exchange','requestedScopes'=>linkedInSafeScopeNames($config['requestedScopes']??$config['scopes']??[]),'httpStatus'=>$status?:null,'durationMs'=>(int)($result['durationMs']??0),'responseIsJson'=>$isJson,'oauthError'=>in_array($linkedinError,['invalid_scope','unauthorized_scope_error','invalid_client','invalid_grant','invalid_redirect_uri','access_denied','server_error','unauthorized','rate_limited'],true)?$linkedinError:($linkedinError===null?null:'unknown'),'oauthErrorCategory'=>in_array($linkedinError,['invalid_scope','unauthorized_scope_error'],true)?'scope':'token_exchange','transportError'=>$transportError],JSON_UNESCAPED_SLASHES));
    ...
    $normalized=strtolower(($linkedinError??'').' '.($linkedinDescription??''));if(in_array($linkedinError,['invalid_scope','unauthorized_scope_error'],true))throw new OAuthException('LINKEDIN_SCOPE_DENIED','LinkedIn hat den angefragten Berechtigungssatz abgelehnt.',422);if($linkedinError==='invalid_client'||$status===401)throw new OAuthException('LINKEDIN_TOKEN_CLIENT','LinkedIn hat die Client-Anmeldedaten abgelehnt.',422);if($linkedinError==='invalid_redirect_uri'||str_contains($normalized,'redirect_uri')||str_contains($normalized,'redirect uri'))throw new OAuthException('LINKEDIN_TOKEN_REDIRECT','Die Redirect URI stimmt nicht mit dem OAuth-Request überein.',422);if($linkedinError==='invalid_grant')throw new OAuthException('LINKEDIN_TOKEN_CODE','Der LinkedIn-Autorisierungscode ist ungültig oder abgelaufen.',422);if($status===429)throw new OAuthException('LINKEDIN_TOKEN_RATE_LIMIT','LinkedIn erlaubt derzeit keinen weiteren Token-Austausch. Bitte später erneut versuchen.',429);if($status<200||$status>=300)throw new OAuthException('LINKEDIN_TOKEN_UPSTREAM','LinkedIn hat den Token-Austausch abgelehnt.',502);if(!$isJson)throw new OAuthException('LINKEDIN_TOKEN_RESPONSE','LinkedIn hat eine ungültige Token-Antwort geliefert.',502);return $decoded;
}
function startLinkedInOAuth(?int $appId=null): never {
    requireAdmin();requireCsrf();$config=linkedInConfig($appId);$state=base64Url(random_bytes(32));$now=time();$states=activeLinkedInOAuthStates($now);$existing=db()->prepare('SELECT 1 FROM social_accounts WHERE provider_app_id=? AND access_token_encrypted IS NOT NULL LIMIT 1');$existing->execute([$config['providerAppId']]);$snapshot=['provider_id'=>'linkedin','provider_app_id'=>$config['providerAppId'],'appName'=>$config['appName'],'credentialVersion'=>$config['credentialVersion'],'created_at'=>$now,'requestedScopes'=>$config['scopes'],'existingConnection'=>(bool)$existing->fetchColumn()];$states[hash('sha256',$state)]=$snapshot;$_SESSION['linkedin_oauth_states']=array_slice($states,-5,null,true);unset($_SESSION['linkedin_oauth']);linkedInOAuthDiagnostic('start',$state,$snapshot,'stored');
...
function consumeLinkedInState(?string $provided): array {
    $now=time();$providedHash=is_string($provided)&&$provided!==''?hash('sha256',$provided):null;$states=activeLinkedInOAuthStates($now);$createdAt=null;$metadata=null;
    if(is_string($providedHash))foreach($states as $storedHash=>$storedAt)if(hash_equals($storedHash,$providedHash)){$metadata=is_array($storedAt)?$storedAt:['created_at'=>$storedAt];$createdAt=$metadata['created_at'];break;}
    ...
    $saved=$metadata??($createdAt===null?null:['created_at'=>$createdAt]);$valid=is_string($provided)&&$provided!==''&&is_string($providedHash)&&$createdAt!==null&&$now-$createdAt<=600&&$createdAt<=$now+30;
    ...
    unset($states[$providedHash],$_SESSION['linkedin_oauth']);$_SESSION['linkedin_oauth_states']=$states;return $saved;
}
...
function linkedInAuthorizationError(?string $error,?string $description): ?string {
    if(!is_string($error)||$error==='')return null;
    if(in_array($error,['access_denied','user_cancelled_login','user_cancelled_authorize'],true))return 'OAUTH_DENIED';
    if(in_array($error,['invalid_scope','unauthorized_scope_error'],true))return 'LINKEDIN_SCOPE_DENIED';
    return 'LINKEDIN_AUTHORIZATION';
}
function linkedInCallback(): never {
    $context=null;
    try{
        $context=consumeLinkedInState(isset($_GET['state'])&&is_string($_GET['state'])?$_GET['state']:null);requireAdmin();
        finishLinkedInCallback($context);
    }catch(OAuthException $e){linkedInRecordOAuthResult($context,'failed',$e->errorCode);throw $e;
    }catch(Throwable $e){linkedInRecordOAuthResult($context,'failed','LINKEDIN_AUTHORIZATION');throw $e;}
}
function finishLinkedInCallback(array $context): never {
    $authorizationError=linkedInAuthorizationError(is_string($_GET['error']??null)?$_GET['error']:null,is_string($_GET['error_description']??null)?$_GET['error_description']:null);if($authorizationError!==null){linkedInRecordOAuthResult($context,'failed',$authorizationError);linkedInAdminRedirect($authorizationError==='OAUTH_DENIED'?'denied':'failed',$authorizationError);}
    ...
    if(($context['provider_id']??null)!=='linkedin'||!is_int($context['provider_app_id']??null))throw new OAuthException('OAUTH_STATE','OAuth-App-Zuordnung fehlt. Bitte erneut verbinden.',403);$config=linkedInConfig($context['provider_app_id']);if(!hash_equals($context['credentialVersion']??'',$config['credentialVersion']))throw new OAuthException('OAUTH_STATE','App-Zugangsdaten wurden während der Autorisierung geändert. Bitte erneut verbinden.',403);$config['requestedScopes']=$context['requestedScopes']??[];$token=linkedInTokenExchange($config,$code);
    ...
    try{
        $lock=$pdo->prepare('SELECT * FROM provider_apps WHERE id=? FOR UPDATE');$lock->execute([$config['providerAppId']]);$currentApp=$lock->fetch();if(!$currentApp||(int)$currentApp['enabled']!==1||!hash_equals($config['credentialVersion'],hash('sha256',$currentApp['client_id'].'|'.$currentApp['client_secret_encrypted'].'|'.$currentApp['redirect_uri'])))throw new OAuthException('OAUTH_STATE','App wurde während der Autorisierung geändert. Bitte erneut verbinden.',403);
        $pdo->prepare('INSERT INTO social_accounts(provider_app_id,provider_id,external_account_id,display_name,account_type,access_token_encrypted,refresh_token_encrypted,token_expires_at,scopes,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE display_name=VALUES(display_name),account_type=VALUES(account_type),access_token_encrypted=VALUES(access_token_encrypted),refresh_token_encrypted=VALUES(refresh_token_encrypted),token_expires_at=VALUES(token_expires_at),scopes=VALUES(scopes),status=VALUES(status),updated_at=VALUES(updated_at)')->execute([$config['providerAppId'],'linkedin',$external,trim($name),'member',$accessEncrypted,$refreshEncrypted,$expiry,$scopes,'connected',$now,$now]);
        $accountStatement=$pdo->prepare("SELECT id FROM social_accounts WHERE provider_app_id=? AND external_account_id=?");$accountStatement->execute([$config['providerAppId'],$external]);$accountId=(int)$accountStatement->fetchColumn();
    ...
    linkedInRecordOAuthResult($context,'success',null,linkedInScopeList($scopes));
