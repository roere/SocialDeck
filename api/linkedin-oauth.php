...
function startLinkedInOAuth(): never {
    requireAdmin();requireCsrf();$config=linkedInConfig();$state=base64Url(random_bytes(32));$now=time();$states=activeLinkedInOAuthStates($now);$snapshot=['created_at'=>$now,'requestedScopes'=>$config['scopes'],'existingConnection'=>(bool)db()->query("SELECT 1 FROM social_accounts WHERE provider_id='linkedin' AND access_token_encrypted IS NOT NULL LIMIT 1")->fetchColumn()];$states[hash('sha256',$state)]=$snapshot;$_SESSION['linkedin_oauth_states']=array_slice($states,-5,null,true);unset($_SESSION['linkedin_oauth']);linkedInOAuthDiagnostic('start',$state,$snapshot,'stored');
...
function finishLinkedInCallback(array $context): never {
    ...
    $config=linkedInConfig();$config['requestedScopes']=$context['requestedScopes']??[];$token=linkedInTokenExchange($config,$code);
