...
function linkedInCallback(): never {
    ...
    $config=linkedInConfig();try{$token=linkedInHttpJson('POST',linkedInEndpoint('token'),['Accept: application/json'],['grant_type'=>'authorization_code','code'=>$code,'client_id'=>$config['clientId'],'client_secret'=>$config['clientSecret'],'redirect_uri'=>$config['redirectUri']]);}catch(OAuthException){throw new OAuthException('LINKEDIN_TOKEN_EXCHANGE','LinkedIn konnte den Authorization Code nicht einlösen.',502);}
