...
function providerDefinitions(): array {
    return [
        'linkedin'=>['id'=>'linkedin','name'=>'LinkedIn','clientIdLabel'=>'Client ID','clientSecretLabel'=>'Client Secret','defaultScopes'=>['openid','profile','w_member_social'],'accountFields'=>['LinkedIn Account-ID'],'capabilities'=>['text'=>true,'image'=>true,'video'=>true,'document'=>true,'article'=>true,'link'=>true,'media'=>['maxFiles'=>1,'mixedMedia'=>false,'articleWithMedia'=>false,'mimeTypes'=>array_keys(array_merge(MEDIA_MIMES['image'],MEDIA_MIMES['video'],MEDIA_MIMES['document'])),'sizeLimits'=>MEDIA_LIMITS]],],
...
function providerDefinition(string $id): ?array { return providerDefinitions()[$id] ?? null; }
