...
export const campaignTests=[
 ...
 ["Kampagne UI Auswahl, Baustein, Overlay, Freigabe, XSS",async()=>{
   ...
   try{
     ...
     const base=root.querySelector('[aria-label="Basis-Antworttext"]');base.value="Hallo ";base.setSelectionRange(6,6);base.dispatchEvent(new Event("input"));base.focus();root.querySelector(".text-block-placeholder").click();assert(base.value==="Hallo {{einladung}}","Baustein");
     ...
     const expand=[...root.querySelectorAll("button")].find(b=>b.textContent.includes("vergrößern"));expand.click();await tick();const enlarged=root.querySelector("dialog[open] textarea");enlarged.value="Overlay-Änderung";enlarged.dispatchEvent(new Event("input"));[...root.querySelectorAll("dialog[open] button")].find(b=>b.textContent==="Schließen").click();await tick();assert(root.querySelector(".campaign-reply textarea").value==="Overlay-Änderung","Overlay State");
