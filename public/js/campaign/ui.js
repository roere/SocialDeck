...
export function createCampaignUI({root,csrf,notify}) {
  ...
  const shell=el("fieldset",undefined,"campaign-shell"),error=el("p",undefined,"form-error"),summary=el("section",undefined,"panel"),feed=el("section",undefined,"panel"),selection=el("section",undefined,"panel"),basePanel=el("section",undefined,"panel"),targets=el("section",undefined,"campaign-targets"),footer=el("section",undefined,"panel");
  error.setAttribute("role","alert");root.append(error,shell);shell.append(summary,feed,selection,basePanel,targets,footer);
  const dialog=el("dialog",undefined,"campaign-overlay"),dialogTitle=el("h2","Kampagnenantwort"),dialogContent=el("div");dialog.setAttribute("aria-label","Kampagnenantwort vergrößert");root.append(dialog);dialog.append(dialogTitle,dialogContent);
  const overlay=createVariantOverlayController({dialog,content:dialogContent,title:dialogTitle});
  const reviewDialog=el("dialog",undefined,"campaign-review"),reviewBody=el("div");reviewDialog.setAttribute("aria-label","Abschlussübersicht und Freigabe");reviewDialog.append(reviewBody);root.append(reviewDialog);
  const manualDialog=el("dialog",undefined,"campaign-manual");manualDialog.setAttribute("aria-label","Beitrag hinzufügen");root.append(manualDialog);
  ...
  const button=(text,fn)=>{const b=el("button",text,"button button-secondary");b.type="button";b.addEventListener("click",()=>run(fn));return b;};
  dialog.prepend(button("Schließen",()=>overlay.close()));
  function change(){dirty=true;renderSelection();renderFooter();}
  function checkbox(text,checked,onChange,disabled=false){const input=el("input");input.type="checkbox";input.checked=checked;input.disabled=disabled;input.addEventListener("change",()=>onChange(input.checked));return label(text,input);}
  function original(item){const box=el("section",undefined,"campaign-original");box.append(el("strong",`${providers.find(p=>p.id===item.provider_id)?.name||item.provider_id} · ${item.author_display_name||"Unbekannter Autor"}`),el("p",date(Object.hasOwn(item,"post_published_at")?item.post_published_at:item.published_at)),el("p",item.post_excerpt||"Kein Beitragsauszug vorhanden.","campaign-excerpt"),link(item.external_post_url));return box;}
  ...
  function renderSummary(){
    summary.replaceChildren(el("h2","Kampagnenübersicht"));
    summary.append(button("Neue Kampagne",()=>{if(dirty&&!confirm("Ungespeicherte Änderungen verwerfen?"))return;campaign=newCampaign();dirty=false;render();}));
    for(const c of campaigns){const active=activeTargets(c),published=c.targets.filter(t=>t.status==="published").length,failed=c.targets.filter(t=>t.status==="failed").length;
      summary.append(button(`${c.name} · ${date(c.created_at)} · ${c.targets.length} Ziele · ${active.length} aktiv · ${published} veröffentlicht · ${failed} fehlgeschlagen · ${c.targets.length-active.length} deaktiviert · ${labels[c.status]}`,async()=>{if(dirty&&!confirm("Ungespeicherte Änderungen verwerfen?"))return;campaign=(await client.load(c.id)).campaign;dirty=false;render();}));
    }
  }
  function renderFeed(){
    feed.replaceChildren(el("h2","Relevante Beiträge"));
    const linked=accounts.filter(a=>a.provider_id==="linkedin");
    if(!linked.some(a=>a.capabilities.read)){feed.append(el("p","LinkedIn-Beiträge können derzeit nicht automatisch eingelesen werden."),el("p","Für den automatischen Abruf ist eine zusätzliche LinkedIn-Berechtigung erforderlich."));}
    feed.append(el("p","V1 sammelt manuell hinzugefügte Beiträge. Bekannte LinkedIn-Posts lassen sich mit gewährtem API-Zugriff einzeln aktualisieren. Ein allgemeiner Netzwerkfeed ist nicht verfügbar.","field-hint"));
    for(const p of providers){const connected=accounts.filter(a=>a.provider_id===p.id);feed.append(el("p",`${p.name}: ${connected.length?connected.map(a=>`${a.display_name||p.name} – Beiträge lesen: ${a.capabilities.postRead?"Ja (bekannte Posts)":"Berechtigung fehlt / nicht verfügbar"}; Kommentare schreiben: ${a.capabilities.write?"Ja":"manuell"}`).join(" · "):"Kein verbundenes Konto; manueller Fallback verfügbar."}`,"field-hint"));}
    ...
    const search=el("input");search.type="search";search.value=keyword;search.placeholder="Auszug / Autor durchsuchen";search.addEventListener("input",()=>{keyword=search.value;renderItems(list);});filters.append(label("Suchwort",search));feed.append(filters,button("Beitrag hinzufügen",showManual));const list=el("div",undefined,"campaign-feed-list");feed.append(list);renderItems(list);
  }
  ...
  function renderSelection(){
    selection.replaceChildren(el("strong",`${campaign.targets.length} Beiträge ausgewählt · ${activeTargets(campaign).length} aktiv`));
  ...
  function renderBase(){
    basePanel.replaceChildren(el("h2","Kampagnenantwort – Basis"));const name=el("input");name.value=campaign.name;name.maxLength=190;name.addEventListener("input",()=>{campaign.name=name.value;change();});
    ...
    const refs=el("div",undefined,"post-text-block-list");for(const block of blocks)refs.append(createTextBlockReferenceButton(block,key=>{const text=activeTextarea?.isConnected&&!activeTextarea.disabled?activeTextarea:base;insertPlaceholderAtSelection(text,key);text.dispatchEvent(new Event("input",{bubbles:true}));}));
    basePanel.append(label("Kampagnenname",name),label("Basis-Antworttext",base),el("h3","Textbaustein einfügen"),refs,el("p","Basisänderungen aktualisieren nur nicht individualisierte Antworten. Textbausteine werden für Vorschau und Freigabe aufgelöst.","field-hint"));
  }
  ...
  function renderTargets(){
    overlay.close(false);targets.replaceChildren(el("h2","Konkrete Antworten / Ziele"));
    const platforms=el("div",undefined,"campaign-platforms");for(const p of providers)platforms.append(checkbox(p.name,campaign.platforms[p.id]!==false,checked=>{campaign.platforms[p.id]=checked;change();renderTargets();}));targets.append(platforms);
    if(!campaign.targets.length)targets.append(el("p","Wähle oben einen oder mehrere Beiträge aus."));
    for(const t of campaign.targets){
      const card=el("article",undefined,"panel campaign-target"),controls=el("section",undefined,"campaign-reply"),locked=["published","publishing"].includes(t.status),active=t.enabled&&campaign.platforms[t.provider_id]!==false;
      ...
      card.append(original(t),controls);targets.append(card);
    }
  }
  function renderFooter(){footer.replaceChildren(el("h2","Kampagnenstatus / Veröffentlichung"),el("p",`${labels[campaign.status]}${dirty?" · Ungespeicherte Änderungen":""}`),button("Kampagne speichern",save),button("Abschlussübersicht prüfen",()=>review()));
    if(campaign.id)footer.append(button("Kampagne löschen",async()=>{if(!confirm("Kampagne mit gespeicherten Antworten löschen? Bereits veröffentlichte Kommentare bleiben im Netzwerk bestehen."))return;await client.remove(campaign.id);campaign=newCampaign();dirty=false;await reloadList();render();}));
  }
  async function review(retryItem){
    ...
    reviewBody.append(button("Zurück zu den Antworten",()=>reviewDialog.close()));reviewDialog.showModal();
  }
  function showManual(){
