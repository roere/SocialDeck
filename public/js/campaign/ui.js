...
export function createCampaignUI({root,csrf,notify}) {
  ...
  function renderFeed(){
    ...
    for(const p of providers){const connected=accounts.filter(a=>a.provider_id===p.id);access.append(el("p",`${p.name}: ${connected.length?connected.map(a=>`${a.display_name||p.name}${a.app_name?" · "+a.app_name:""} – Beiträge lesen: ${a.capabilities.postRead?"Ja (bekannte Posts)":"Berechtigung fehlt / nicht verfügbar"}; Kommentare schreiben: ${a.capabilities.write?"Ja":"manuell"}`).join(" · "):"Kein verbundenes Konto; manueller Fallback verfügbar."}`,"field-hint"));}
  ...
  function renderItems(list){
    ...
    for(const item of found){const card=el("article",undefined,"post-list-item campaign-feed-card"),t=campaign.targets.find(t=>Number(t.engagement_item_id)===Number(item.id));
      ...
      const account=accounts.find(a=>a.provider_id===item.provider_id&&a.enabled&&a.capabilities.postRead);
  ...
  async function review(retryItem){
    ...
    for(const t of r.targets){const details=el("details");details.append(el("summary",`${t.provider_id} · ${t.author_display_name}${t.app_name?" · App: "+t.app_name:""} · ${t.publishable?"API-Veröffentlichung":"Manuelle Veröffentlichung erforderlich"}`),original(t),el("pre",t.reply_text,"campaign-excerpt"));reviewBody.append(details);}
